...
class MultiHeadedAttentionDe(nn.Module):
    def __init__(self, head_num, d_model, dropout=0.0):
        super(MultiHeadedAttentionDe, self).__init__()
...
class PositionwiseFeedForwardDe(nn.Module):
    def __init__(self, d_model, d_ff, dropout=0.0):
        super(PositionwiseFeedForwardDe, self).__init__()
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.dropout(x)  # x的词向量与位置向量相加之后还加入了dropout
...
def positional_encodings_like(x, t=None):   # hope to be differentiable
    ...
    channels = torch.true_divide(torch.arange(0, x.size(-1), 2), x.size(-1))
    if x.is_cuda:
